...
function initEditor(){
    ...
    $popEditor.find('#costpId').simSelect({
		callback: function (x,v) {
		if(!x) return;
			$(this).find("input:first").val(v);
			var costunit = $(this).find('li[val='+v+']').find("input[class='hideText']").val();
			$popEditor.find("input[name='costunit']").val(costunit);
		}
    });
    $popEditor.find('#consIcons').simSelect({
		callback: function (x,v) {
			if(!x) return;
			var $this = $(this);
			$this.find("input:first").val(v);
            $this.parent('.popLine').siblings('.xznhIconBox').find('.ghPic').removeClass().addClass('ghPic '+v+''); 
		}
    });
}
</script>
	<div class="hd">
		...
		<input type="hidden" name="iotype" value="0" />
	...
	<div class="popBg">
		...
		<div class="popLine mtb4">
			<span class="popLabelStyle w118">成本属性：</span>
			<div class="mod-select popSelStyle w243" id="costpId">
				<p class="text">选择成本</p>
				<span class="arr-group hand"> <i class="icon-mod icon-arr"></i></span>
				<input type="hidden" name="costpid" value="${homeio.costpid}" />
				<div class="list">
					<ul>
						<c:forEach var="i" items="${costs}">
							<li val="${i.classpropertyid}"><a href="">${i.classpropertyname}
								<input type="hidden" class="hideText" value="${i.unitname}"/>
								</a>
							</li>
						</c:forEach>
		...
		<div class="popLine mtb4">
			<span class="popLabelStyle w118">成本单位：</span>
			<input class="popInputStyle w274" type="text" name="costunit" value="${homeio.costunit}">
			<span class="popCheckMsg"></span>
		</div>
		<div class="popLine mtb4">
			<span class="popLabelStyle w118">图片：</span>
			<div class="mod-select popSelStyle w243" id="consIcons">
				<p class="text">选择图片</p>
				<span class="arr-group hand"> <i class="icon-mod icon-arr"></i></span>
		...
		<div class="popLine mtb4 xznhIconBox">
            <div class="ghPic ${homeio.energyicon}"></div>
		</div>
	</div>
