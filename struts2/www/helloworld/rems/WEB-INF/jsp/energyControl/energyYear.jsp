...
<script type="text/javascript">
	...
	$(function(){
		...
		function setSession(pid){
			$.ajax({
				type:"GET"
				,url:"<c:url value='/analyse/run/session' />?date="+new Date().getTime()
				,data:{pid:pid}
			});
		}
		//提交表单数据生成列表
		$("#dateListduId").click(function(){
			var type = $('.tab-nav .on','#energyLeftId').attr('val'),$tab;
		...
		$.ajax({ 
			...
			,data: analyseData
			,url: "<c:url value='/analyse/energyYearContent' />"
			,success: function(data){
				$("#dataListContentId").empty().html(data);
				$("#excelYearId").css("display","block");
			}});
		});
		//提交表单数据生成柱状图
		$("#dateZhuId").click(function(){
			$.ajax({ 
				type:"POST"
				,data: $("#analyseForm").serialize()
				,url: "<c:url value='/analyse/energyYearZhu' />"
				,success: function(data){
					$("#dataListContentId").empty().html(data);
					$("#excelYearId").css("display","none");
				}
	...
	$('#excelYearId').click(function(){
		...
		var analyseData = getData($("#analyseForm"),type);
		$.simDownload("<c:url value='/analyse/energyYearExport1' />",analyseData,'post');
	});
		//导出
		/*$("#excelYearId").click(function(){
			if (validdata()) {
			...
			var url="<c:url value='/analyse/energyYearExport1' />";
		...
		*/
		...
		function energyLeft(propertyid){
			...
			$.ajax({ 
				type:"POST"
				,data: {id:propertyid}
				,url: "<c:url value='/analyse/energyLeftYear' />"
				,success: function(data){
                    ...
                    <table class="putform">
                        <tbody><tr>
                            ...
                            <td class="tleft">
                                ...
                                <a class="fr execl mt5" id="excelYearId" href="#" style="display:none;"></a>
                            </td>
                        </tr>
                    </tbody></table>
                </div>
